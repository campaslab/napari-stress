...
import tqdm
import vedo
from napari.types import ImageData, LayerDataTuple, PointsData, SurfaceData
...
def surface_to_list_of_surfaces(surface: SurfaceData) -> list:
    ...
    for t in range(n_frames):
        ...
        _faces = faces[idx_face_new_frame[t] : idx_face_new_frame[t + 1] - 1] - sum(
            points_per_frame[:t]
        )
...
def surf_fit(points, Xq, **kwargs):
    ...
    xgrid = np.mgrid[x.min() - sL : x.max() + sL : sL, y.min() - sL : y.max() + sL : sL]
...
def mean_curvature(z, spacing):
    ...
    H = (
        (1 / 2.0)
        * ((1 + Zxx**2) * Zyy - 2.0 * Zx * Zy * Zxy + (1 + Zyy**2) * Zxx)
        / (1 + Zxx**2 + Zyy**2) ** (1.5)
    )
