...
def test_rescaling4d():
    ...
    resampled_image = rescale(image, scale_z=1.0, scale_y=1.0, scale_x=1.0)
    assert np.array_equal(
        resampled_image.squeeze().shape, image.squeeze().shape
    )

    # Test for 2D
    image = np.ones((100, 100))
    resampled_image1 = rescale(
        image, scale_z=1.0, scale_y=1.0, scale_x=0.5
    ).squeeze()
    resampled_image2 = rescale(
        image, scale_z=1.0, scale_y=0.5, scale_x=1.0
    ).squeeze()
    resampled_image3 = rescale(
        image, scale_z=0.5, scale_y=1.0, scale_x=1.0
    ).squeeze()
