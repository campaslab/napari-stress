...
def geodesic_distance_matrix(surface: SurfaceData) -> np.ndarray:
    ...
    distance_matrix = np.triu(np.ones((n_points, n_points)))
...
def geodesic_analysis(surface: SurfaceData,
    ...
    ----------
    anisotropic_stress: np.ndarray
        anisotropic stress at every point of droplet surface
    anisotropic_stress_cell: np.ndarray
        anisotropic stress - anisotropic stress on ellipsoid
    anisotropic_stress_tissue: np.ndarray
        anisotropic stress on surface of LSQ ellipsoid
    ...
    min_max_distances = local_min_max_and_dists(surface[0], anisotropic_stress, maximal_distance)
    min_max_distances_cell = local_min_max_and_dists(surface[0], anisotropic_stress_cell, maximal_distance)
    ...
    # cdf_info = CDF_Analysis_of_Data(anisotropic_stress, alpha_total)
    ...
    # cdf_info_cell = CDF_Analysis_of_Data(anisotropic_stress_cell, alpha_cell)
    ...
    # cdf_info_autocorrelations = CDF_Analysis_of_Data(autocorrelations['All_local_min_max_pairs_anisotropies'],
    ...
    # metadata = {'autocorrelations_total': autocorrelations,
    #             'autocorrelations_cellular': autocorrelations_cell,
    #             'autocorrelations_tissue': autocorrelations_tissue}
    # return metadata
...
def correlation_on_surface(surface: SurfaceData,
                           feature1: np.ndarray,
                           feature2: np.ndarray,
    ...
    if distance_matrix is None:
        distance_matrix = geodesic_distance_matrix(surface)
    ...
    for dist_i in range(0, maximal_distance+1):
        ...
        sum_mean_curv_corr_d_i, num_mean_curv_corr_d_i = avg_around_pt(dist_i, dists_lbdv_non0, Corr_non0_pts, maximal_distance)
...
def CDF_Analysis_of_Data(data: np.ndarray,
...
def local_min_max_and_dists(points: PointsData,
                            feature: np.ndarray,
                            maximal_distance: float):
    """
    Find local minima and maxima of surface feature and inbetween distances.
    ...
    nearest_min_max_anisotropic_stress = np.zeros_like(feature)  # 2*(H_in_max - H_in_min), from extrema to nearest partner
    ...
    for pt in range(quad_fit):
        ...
        tris_containing_pt = np.where( points == pt )

        rows_pt = tris_containing_pt[0]
        num_occ_pt = np.count_nonzero(points == pt)
        ...
        for row_pt in range(len(rows_pt)):
            ...
            for other_tri_pt_num in range(3):
                other_tri_pt =     int(points[row_num_pt, other_tri_pt_num])

                H_other_tri_pt = feature[other_tri_pt, 0]
    ...
    for pt_max in range(num_local_max):
        ...
        local_max_field = feature[pt_max_num, 0]

        for pt_min in range(num_local_min):
            pt_min_num = local_min_inds[pt_min]
            local_min_field = feature[pt_min_num, 0]

            dist_max_min_pt = maximal_distance[pt_min_num, pt_max_num]
    ...
    for pt_max in range(num_local_max):
        ...
        local_min_dists_to_pt = maximal_distance[pt_max_num, local_min_inds]
        ...
        nearest_min_max_anisotropic_stress[pt_max_num] = ( feature[pt_max_num, 0] - feature[pt_num_of_nearest_min, 0] )

    for pt_min in range(num_local_min):
        pt_min_num = local_min_inds[pt_min]

        local_max_dists_to_pt = maximal_distance[pt_min_num, local_max_inds]
        ...
        nearest_min_max_anisotropic_stress[pt_min_num] = ( feature[pt_num_of_nearest_max, 0] - feature[pt_min_num, 0] )

    result = {
        'local_maxima_and_minima': local_max_and_min,
        'nearest_min_max_distances': nearest_min_max_dists,
        'nearest_min_max_anisotropic_stress': nearest_min_max_anisotropic_stress,
        'All_local_min_max_pairs_distances': All_local_min_max_pairs_distances,
        'All_local_min_max_pairs_anisotropies': All_local_min_max_pairs_anisotropies
        }
